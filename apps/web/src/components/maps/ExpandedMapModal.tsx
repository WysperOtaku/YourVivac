...
export function ExpandedMapModal({ open, onClose, title, center, zoom, layer, marks, route, footer }: Props) {
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[60] flex flex-col sm:p-2">
      <div className="absolute inset-0 hidden bg-black/60 sm:block" onClick={onClose} />
      {/* Móvil: pantalla completa. Escritorio: casi pantalla completa (card enorme). */}
      <div className="relative z-10 mx-auto flex h-full w-full flex-col overflow-hidden bg-bg-2 sm:max-w-none sm:rounded-card sm:shadow">
